// un fichier de test doit s'appeler <nom du dossier à tester>_test.go
// Bonne pratique de rédiger les tests au fur et à mesure du développement
// permets dans ce cas de mettre en évidence l'intéret d'avoir séparé la logique de jeu
...
func TestGameHasWon(t *testing.T) {
	g, _ := New(3, "bob")
	g.MakeAGuess("b")
	g.MakeAGuess("o")
...
func TestGameHasLost(t *testing.T) {
	g, _ := New(3, "bob")
	g.MakeAGuess("r")
	g.MakeAGuess("a")
	g.MakeAGuess("t")
